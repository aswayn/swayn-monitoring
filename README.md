...
2. Configure domain resolution:
   ```bash
   # Add to /etc/hosts (replace with your server IP if remote)
   echo "127.0.0.1 giants.corp.swayn.com vault.corp.swayn.com" >> /etc/hosts
   ```
...
3. Run the setup script:
   ...
   ./install.sh
   ```
   This will automatically generate SSL certificates and create all configuration files.

4. Configure environment variables:
   ```bash
   cp .env.example .env
   # Edit .env to set your desired passwords and tokens
   ```
...
### Access URLs
- **Grafana**: https://giants.corp.swayn.com/grafana/ (admin/admin by default)
...
- **Domain**: `giants.corp.swayn.com`, `vault.corp.swayn.com` (specific domains)
...
- Certificate Signing Request: `configs/nginx/ssl/server.csr`
- Backup (old wildcard): `configs/nginx/ssl/server-old.crt` and `server-old.key`
...
- RSA 4096-bit private keys
- Certificate covers: giants.corp.swayn.com, vault.corp.swayn.com
- Self-signed certificates (automatically generated by install.sh)
...
All services are accessible via `giants.corp.swayn.com`:
- Add `127.0.0.1 giants.corp.swayn.com` to your `/etc/hosts` file
- Or configure DNS to point to your server IP
...
**Send logs via TCP:**
```bash
echo "<14>$(date '+%b %d %H:%M:%S') myhost myapp[123]: Test log message" | nc giants.corp.swayn.com 1514
```

**Send logs via UDP:**
```bash
echo "<14>$(date '+%b %d %H:%M:%S') myhost myapp[123]: UDP log message" | nc -u giants.corp.swayn.com 1514
```
...
```bash
docker run --log-driver syslog --log-opt syslog-address=tcp://giants.corp.swayn.com:1514 your-app
```
...
```bash
curl -X POST http://giants.corp.swayn.com:3001/api/keys/generate-ssh \
...
**Retrieve Public Key:**
```bash
curl -X GET http://giants.corp.swayn.com:3001/api/keys/1 \
  -H "Authorization: Bearer <your-jwt-token>"
```

**Get Encrypted Private Key:**
```bash
curl -X POST http://giants.corp.swayn.com:3001/api/keys/1/private \
